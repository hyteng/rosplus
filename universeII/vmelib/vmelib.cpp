...
int VMEBridge::rl(int image, unsigned int addr, unsigned int *data, int size)
{
    ...
    //if (pread(vme_handle[image], data, size, (addr - vmeBaseAddr[image])) != size) {
...
int VMEBridge::wl(int image, unsigned int addr, unsigned int *data, int size)
{
    ...
    //if (pwrite(vme_handle[image], data, size, (addr - vmeBaseAddr[image])) != size) {
...
int VMEBridge::rw(int image, unsigned int addr, unsigned short *data, int size)
{
    ...
    int readSize = pread(vme_handle[image], data, size, (addr - vmeBaseAddr[image]) | 0x20000000);
    if (readSize != size) {
        *Err << "Bus error reading at address 0x" << hex << addr << dec
            << " readSize: " << readSize << ", image " << image << "!\n";
        return -1;
    }
...
// Disable image and check that device is accessible
    ...
    *Std << "vmemap. base:" << hex << imageRegs.base << " size: " << imageRegs.size  << dec << "\n";
    ret = ioctl(vme_handle[image], IOCTL_SET_IMAGE, &imageRegs);

	if (ret < 0) {
		*Err << "Error: Failed to allocate Image " << image << "! code: " << ret << "\n";
        bridge_error = -7;
        return -7;
    }
...
//  Try to allocate a new image;  ms = 0: master image, ms = 1: slave image
    ...
    ioctl(uni_handle, IOCTL_TEST_BERR);
...
// setup registers CTL, TO, BD, BS of image
    ...
    if (ms) {   // Slave image
        ...
        ctl = 0x00000000 | vdw | vas;   // enable posted write, non-priv. data
